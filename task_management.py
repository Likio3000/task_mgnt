...
class TaskManagement:
    def __init__(self, parent, progress):
        ...
        self.delete_button = ttk.Button(self.parent, text="Delete Selected", command=self.delete_selected)
        self.delete_button.pack(pady=10)

        self.clear_completed_button = ttk.Button(self.parent, text="Mark Completed Tasks", command=self.mark_completed_tasks)
    ...
    def load_activities(self):
        ...
        self.filter_activities_by_date(current_date)

    def filter_activities_by_date(self, date):
        for row in self.tree.get_children():
            self.tree.delete(row)
        activities = get_activities_from_db(date)
        now = datetime.now()
